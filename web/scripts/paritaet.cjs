...
const { model, jahr, faelle } = JSON.parse(fs.readFileSync(datei, "utf8"));
...
for (const f of faelle) {
  ...
  const x = baueFeatures(model, f.a, f.b, f.rating_a.elo, f.rating_b.elo,
  ...
  if (x.length !== f.erwartet.merkmale.length) {
    ...
    x.forEach((v, i) => {
      ...
      if (!gleich(v, f.erwartet.merkmale[i])) {
        const name = model.features[i] ?? `Merkmal #${i} (nicht im ausgelieferten Modell)`;
  ...
  const p = prognostiziere(model, f.a, f.b, f.rating_a.elo, f.rating_b.elo,
    f.rating_a.n_gaenge, f.rating_b.n_gaenge, h2hTs).p;
  model.klassen.forEach((kl, k) => {
