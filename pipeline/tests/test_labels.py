...
def test_dedup_abzeichen_wird_pro_seite_uebernommen():
    roh = [
        RohGangEintrag("ev1", "2024-05-01", "anna|1995", "beat|1996", "+", 10.0, "kantonal", status_abzeichen=True),
        RohGangEintrag("ev1", "2024-05-01", "beat|1996", "anna|1995", "o", 8.75, "kantonal", status_abzeichen=False),
    ]
    gaenge, _ = dedupliziere(roh)
    g = gaenge[0]
    assert g.schwinger_a_id == "anna|1995"  # kanonische A-Seite
    assert g.status_abzeichen_a is True
    assert g.status_abzeichen_b is False


def test_dedup_abzeichen_default_false():
    roh = [
        RohGangEintrag("ev1", "2024-05-01", "anna|1995", "beat|1996", "+", 10.0, "kantonal"),
        RohGangEintrag("ev1", "2024-05-01", "beat|1996", "anna|1995", "o", 8.75, "kantonal"),
    ]
    gaenge, _ = dedupliziere(roh)
    assert gaenge[0].status_abzeichen_a is False
    assert gaenge[0].status_abzeichen_b is False


def test_dedup_einzelne_perspektive_abzeichen_nur_fuer_vorhandene_seite():
    roh = [
        RohGangEintrag("ev1", "2024-05-01", "anna|1995", "beat|1996", "+", 10.0, "regional", status_abzeichen=True),
    ]
    gaenge, _ = dedupliziere(roh)
    g = gaenge[0]
